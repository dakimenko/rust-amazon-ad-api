...
pub async fn list_campaigns(
    ...
    let req = configuration.client.request(reqwest::Method::POST, format!("{}/sb/v4/campaigns/list", configuration.base_path))
...
pub async fn create_campaigns(
    ...
    let req = configuration.client.request(reqwest::Method::POST, format!("{}/sb/v4/campaigns", configuration.base_path))
        .header("Accept", VERSION_HEADER)
        .header("Content-Type", VERSION_HEADER)
        .json(&body)
        .build()?;
...
pub async fn update_campaigns(
    ...
    let req = configuration.client.request(reqwest::Method::PUT, format!("{}/sb/v4/campaigns", configuration.base_path))
        .header("Accept", VERSION_HEADER)
        .header("Content-Type", VERSION_HEADER)
        .json(&body)
        .build()?;
